...
#Sampling params
N_TX = 150      #Number of points in t and x dimension
N_V = 120       #Number of points in v
...
for epoch in range(EPOCHS):
    ...
    x = torch.rand(N_TX, 1, device=DEVICE) * (X_MAX - X_MIN) + X_MIN  # [-2,2]

    txv = make_txv_stack(t, x, v)

    loss = 0.1 * pde_loss(model, txv, TAU) + 10 * ic_loss(model, x, v) + boundary_loss(model, t, v, X_MIN, X_MAX) #MAGIC NUMBER!
    print(loss.item())
...
_, x, v, domain = initialize_physics_data(N_TX, N_V, T_MIN, T_MAX, X_MIN, X_MAX, V_MAX, DEVICE, DTYPE, SEED)
